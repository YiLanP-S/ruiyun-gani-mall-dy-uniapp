      ...
      <tn-form :model="form" ref="form" :rules="rules" label-width="150">
        ...
        <tn-form-item label="地区" prop="areaName">
          <view style="display: flex; align-items: center" @tap="showArea = true">
            <input placeholder-style="color: #AAAAAA" style="flex: 1" v-model="form.areaName" placeholder="选择省市区" disabled readonly />
        ...
        <tn-form-item class="flexTop" label="详细地址" prop="detailAddress" @tap="chooseLocation" v-if="isDisabled">
          <textarea placeholder-style="color: #AAAAAA" v-model="form.detailAddress.split('•')[0]" placeholder="选择详细地址(街道门牌、楼层等信息)" :disabled="isDisabled" readonly />
        </tn-form-item>
      ...
      this.$refs.form
        ...
        .then(res => {
          ...
          if (res) {
            ...
            that.$http.postRequest('app-api/member/address/create', that.form).then(res => {
              if (res.code == 0) {
