<template>
  <view class="oa-content">
    ...
    <view class="tn-padding-bottom-xl" style="padding-top: 100rpx">
      <view class="j-2">
        <view class="j-3" v-for="(item, index) in list" :key="index">
          ...
          <view class="j-10">
            ...
            <view class="j-12">
              <tn-button class="tn-button" backgroundColor="#0059FF" fontColor="#ffffff" style="min-width: 180rpx" shape="round" v-if="item.buttons.length === 0" @click="go2Detail(item.id)">查看详情</tn-button>
              ...
              <!-- 		<tn-button class="tn-button" backgroundColor="#0059FF" fontColor="#ffffff" style="min-width: 180rpx;" shape="round" v-if="item.buttons.includes('afterSales')" @click="tn('./comment/add?id='+item.id)">申请售后</tn-button> -->
            </view>
          </view>
        </view>
      </view>
      ...
      <!-- 联系客服 -->
      <!-- <view v-if="list.length == 0" style="padding: 0 30rpx; position: fixed; left: 0; bottom: 40rpx; width: 100%">
        <view class="c-2">
...
export default {
  ...
  onLoad(options) {
    console.log('页面切换时onLoad', options)
    if (options.status) {
  ...
  methods: {
    ...
    getlist() {
      ...
      let than = this
      than.$http.getRequest('app-api/trade/order/page', data).then(res => {
        if (res.code == 0) {
