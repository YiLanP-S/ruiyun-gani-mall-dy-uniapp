...
<!-- 				<view class="d-3" v-if="logs[logs.length-1].content">
				...
				<!-- <view class="c-2" style="margin-top: 30rpx;" v-if="detail.status>22">
					<view class="d-11-box">
						<view class="d-11">
							...
							<view class="d-13">
								...
								<view class="d-15">
									<view class="d-16">接单<text>{{workerInfo.work.count}}</text>次</view>
				...
				<view class="c-2">
					<view class="c-15" v-for="item in detail.items" :key="item.id">
						...
						<!-- 	<view style="display: flex;gap: 20rpx;margin-top: 5rpx;">
								<tn-button class="tn-button" shape="round" backgroundColor="#0059FF" :fontSize="24" fontColor="#FFFFFF" size="sm" v-if="[30].includes(detail.status) && item.afterSaleStatus === 0" @click="tn('./afterSales/apply?orderId='+id+'&itemId='+item.id)">
									<text class="">申请售后</text>
								</tn-button>
								<tn-button class="tn-button" shape="round" backgroundColor="#0059FF" :fontSize="24" fontColor="#FFFFFF" size="sm" v-if="detail.status==30 && detail.orderTag!='1'" @click="tn('./quality/quality?orderId='+id+'&itemId='+item.id)">
...
<!-- 						<view class="c-26">
						...
						<view class="c-29">
							<text class="c-30">分销佣金：</text>
							<text class="c-31">￥{{ parseFloat((detail.distributorPrice / 100).toFixed(2)) }}</text>
						</view>
					</view>
				</view>
	<!-- 			<view class="c-2">
				  ...
				  <scroll-view class="c-77" scroll-x>
					  <view class="c-78-box">
						  <view :class="['c-78',active_index == index?'active':'']" v-for="(item,index) in days" :key="index" @click="switchDay(index)">
							  <view class="c-79-box">
								  <view class="c-79">{{index==0?'今天':index==1?'明天':index==2?'后天':item[1]}}</view>
			  				...
			  				<view class="c-65">
			  					<view class="c-69" style="width: 100%;">
			  						<tn-button class="tn-button" shape="round" backgroundColor="#0059FF" padding="40rpx 0" width="100%" :fontSize="28"
			  							fontColor="#FFFFFF" @click="reservation">
	...
	export default {
		...
		methods: {
			...
			reservation(){
				 ...
				 this.appointEndTime = `${dateTime.getFullYear()}-${('0' + (dateTime.getMonth() + 1)).slice(-2)}-${('0' + dateTime.getDate()).slice(-2)} ${('0' + dateTime.getHours()).slice(-2)}:${('0' + dateTime.getMinutes()).slice(-2)}:${('0' + dateTime.getSeconds()).slice(-2)}`;
			...
			getDetail1(){
				this.fileList = []
				this.$http.getRequest('app-api/trade/order/get-detail-by-bizId',{
					bizId:this.bizId
				})
				.then(res => {
					if(res.code == 0){
			...
			getOrderLog(){
				this.$http.getRequest('app-api/trade/order/get-order-log',{
					id:this.id
				})
				.then(res => {
					if(res.code == 0){
			...
			getWorker(){
				this.$http.getRequest('app-api/trade/work/getWorkStatusLog',{
					workId:this.id
				})
				.then(res => {
					if(res.code == 0){
			...
			uploadSuccess(data, index, lists){
				...
				if(data.code == 0 ){
					...
					this.$http.postRequest('app-api/trade/order/update-security-images',{
						orderId:this.id,
