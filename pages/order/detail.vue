  ...
  <view class="oa-content" :style="{ paddingBottom: vuex_is_black_stripe ? 'calc(20px + 120rpx)' : '120rpx' }">
    ...
    <view class="tn-padding-bottom-xl">
      <view class="d-1">
        ...
        <view class="d-3" v-if="detail.status === 22"> 正在等待师傅接单，请耐心等待 </view>
        ...
        <view class="c-2" style="margin-top: 30rpx" v-if="detail.status > 22 && detail.status != 27">
          <view class="d-11-box">
            <view class="d-11">
              <image v-if="workerInfo.work && workerInfo.work.workerPicUrl" :src="workerInfo.work.workerPicUrl" class="d-12" />
              <image v-else src="https://gani-sglm.oss-cn-hangzhou.aliyuncs.com/mf0019.png" class="d-12" />
              <view class="d-13">
            ...
            <view class="callPhoneBox" @click="callPhone">
              <view class="callPhoneBox_t"></view>
              <view class="callPhoneBox_c">联系师傅</view>
            </view>
          </view>
        </view>
        ...
        <view class="c-2">
          <view class="c-15" v-for="item in detail.items" :key="item.id">
            <image v-if="item.picUrl" class="c-16" :src="item.picUrl" />
            <image v-else src="https://gani-sglm.oss-cn-hangzhou.aliyuncs.com/mf0022.png" mode="aspectFit|aspectFill|widthFix" class="c-16" lazy-load="true" />
            <view class="c-17">
    ...
    <view class="tn-flex tn-footerfixed tn-padding" :style="{ paddingBottom: vuex_is_black_stripe ? 'calc(20px + 20rpx)' : '' }" v-if="detail.buttons.length">
      <view class="tn-flex tn-flex-1 justify-content-item tn-text-center c-38">
        <tn-button
          ...
          :fontSize="28"
          fontColor="#FFFFFF"
          v-if="detail.buttons.includes('cancel') || detail.buttons.includes('pay')"
          @click="isLogin ? continueToPay() : login()"
        >
...
export default {
  ...
  methods: {
    ...
    continueToPay() {
      let that = this
      console.log('继续支付', typeof that.detail.id)
      tt.continueToPay({
        orderId: '', // 内部订单号
        outOrderNo: that.detail.id.toString(), // 外部订单号 2个订单号必填一个
        success: res => {
          console.log('支付success res', res)
          // 调用接口获取支付结果
          // this.getDetail()
          uni.redirectTo({
            url:
...
.callPhoneBox {
  .callPhoneBox_t {
    width: 48rpx;
    height: 48rpx;
    background: url(https://gani-sglm.oss-cn-hangzhou.aliyuncs.com/mf0017.png);
