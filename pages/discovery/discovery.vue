<template>
  <scroll-view class="page-c" scroll-y="true" @scrolltolower="onScrollToLower">
    <!-- 顶部自定义导航 -->

<!--    <tn-nav-bar fixed customBack :bottomShadow="false" backgroundColor="#FFFFFF">
      <view class="tn-flex tn-flex-col-center tn-flex-row-center ">
        <text class="tn-text-bold tn-text-xl tn-color-black">案例</text>
      </view>
    </tn-nav-bar> -->
    ...
    <view class="tn-flex tn-flex-direction-column tn-margin-bottom-xl">
      ...
      <block v-for="(item,index) in list" :key="index">
        <view class="tn-blogger-content2__wrap" @click="tn('./case?id='+item.id)">
          <view class="tn-shadow-blur image-pic" :style="'background-image:url(' + item.picUrl + ');background-size: cover;background-position: center;'">
            <view class="image-design">
              <view class="tn-text-df" style="width: 100%;height: 120rpx;background: linear-gradient(0deg, rgba(0,0,0,0.3), rgba(0,0,0,0));position: absolute;bottom: 0;border-radius: 0 0 12rpx 12rpx;">
                <view class="tn-text-lg tn-text-bold tn-padding-top-xl tn-padding-left tn-padding-right tn-color-white clamp-text-1">{{ item.name }}</view>
              </view>
            </view>
          </view>  
	...
	<view class="tn-flex tn-flex-direction-column tn-margin-bottom-xl">
	  ...
	  <block v-for="(item,index) in list" :key="index">
	    <view class="tn-blogger-content2__wrap" @click="tn('/pages/discovery/case?id='+item.id)">
	      <view class="tn-shadow-blur image-pic" :style="'background-image:url(' + item.picUrl + ');background-size: cover;background-position: center;'">
	        <view class="image-design">
	          <view class="tn-text-df" style="width: 100%;height: 120rpx;background: linear-gradient(0deg, rgba(0,0,0,0.3), rgba(0,0,0,0));position: absolute;bottom: 0;border-radius: 0 0 12rpx 12rpx;">
	            <view class="tn-text-lg tn-text-bold tn-padding-top-xl tn-padding-left tn-padding-right tn-color-white clamp-text-1">{{ item.name }}</view>
	  ...
	  <view v-if="list.length==0" class="wu" style="margin-top: 100rpx;">
	  	 <tn-empty icon="https://www.tmsglm.com/statics/2.png" text="暂无数据"></tn-empty>
	  </view>
	</view>
	<!-- #endif -->
     <tn-load-more v-if="list.length>0" :status="loadStatus" :loadText="loadText"></tn-load-more>
 ...
 export default {
    ...
    methods: {
	  ...
	  getList() {
	  		...
	  		that.$http.getRequest('app-api/official/example-info/page',{
	  			pageNo:that.pageNo,
	  			pageSize:10,
	  			status:0
	  		})
