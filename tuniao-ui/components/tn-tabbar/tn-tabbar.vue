<template>
  <view v-if="show" class="tn-tabbar-class tn-tabbar" @touchmove.stop.prevent="() => {}">
    ...
    >
      ...
      >
        ...
        <view :class="[itemButtonClass(index)]" :style="[itemButtonStyle(index)]">
          ...
          <!-- 角标-->
          <tn-badge v-if="!item.out && (item.count || item.dot)" :dot="item.dot || false" backgroundColor="tn-bg-red" fontColor="#FFFFFF" :radius="item.dot ? 14 : 0" :fontSize="14" padding="2rpx 4rpx" :absolute="true" :top="2">
    ...
    <!-- 防止tabbar塌陷 -->
    <view
      class="tn-tabbar__placeholder"
      :class="{ 'tn-safe-area-inset-bottom': safeAreaInsetBottom }"
      :style="{
...
export default {
  ...
  created() {
    ...
    this.$t.vuex('safeAreaBottom', screenHeight - safeArea.bottom)
  },
  mounted() {
    this.$nextTick(() => {
      this.getTabbarItem()
  ...
  methods: {
    ...
    switchTab(index) {
      // 发出事件和修改v-model绑定的值
      this.$emit('change', index, this.list[index].pagePath)
