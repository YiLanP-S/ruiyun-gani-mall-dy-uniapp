      ...
      <scroll-view scroll-x class="tn-tabs__scroll-view" :scroll-left="scrollLeft" scroll-with-animation>
        <view class="tn-tabs__scroll-view__box" :class="{'tn-tabs__scroll-view--flex': !isScroll}">
          ...
          >
            ...
            {{ item[name] || item['name'] }}
			<view v-if="(isHome&&isSort&&index == 1)||(!isHome&&isSort)" class="sx_box">
				<text :style="{'display': 'block','color':(item.sort=='asc'&&active_index==index)?'#0059FF':''}" class="tn-icon-up-triangle"></text>
				<text :style="{'display': 'block','margin-top': '-15rpx','color':(item.sort=='desc'&&active_index==index)?'#0059FF':''}" class="tn-icon-down-triangle"></text>
			</view>
          </view>
          <!-- 底部滑块 -->
  ...
  export default {
    ...
    methods: {
      ...
      clickTab(index) {
        ...
        if (index === this.currentIndex && !this.isSort) return
